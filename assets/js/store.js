...
function initHeroScrollAnimation(){
  ...
  const animated=[device,frame,brand,subBrand,tagline,island].filter(Boolean);
  animated.forEach(el=>{
  ...
  function readTarget(){
    ...
    target=Math.max(0,Math.min(1,-rect.top/distance));
    if(!running){
      running=true;
  ...
  function render(p){
    const scale=1-.065*p;
    device.style.transform=`translate3d(0,${-58*p}px,0) scale3d(${scale},${scale},1)`;
    device.style.opacity=String(1-.08*p);
    frame.style.transform=`translate3d(0,${54-18*p}px,0) rotateX(${9-3*p}deg) rotateZ(${8-11*p}deg)`;
    if(brand){
      brand.style.transform=`translate3d(0,${-10*p}px,0)`;
      brand.style.opacity=String(1-.12*p);
    }
    if(subBrand)subBrand.style.transform=`translate3d(0,${-4*p}px,0) scale3d(${1+.035*p},${1+.035*p},1)`;
    if(tagline){
      tagline.style.transform=`translate3d(0,${8*p}px,0)`;
      tagline.style.opacity=String(1-.35*p);
    }
    if(island)island.style.transform=`translateZ(0) scale3d(${1-.08*p},1,1)`;
  }

  function tick(){
    const delta=target-current;
    current += delta*.18;
    if(Math.abs(delta)<.00035) current=target;
    render(current);
    if(current!==target){
      raf=requestAnimationFrame(tick);
    }else{
  ...
  addEventListener("resize",readTarget,{passive:true});
  current=target=0;
  readTarget();
}
...
function initPhoneFinder(){
 ...
 section.innerHTML=`<a class="case-studio-showcase reveal" href="setup.html" aria-label="Customize your own phone case">
   <div class="case-show-copy">
     <span class="eyebrow">HADI CASE STUDIO</span>
     <h2>Your case. <span>Your design.</span></h2>
     <p>Upload a photo, add your text, choose the case color and preview your creation in 3D.</p>
     <span class="case-show-cta"><span>Start designing</span><b>→</b></span>
   </div>
   <div class="case-show-visual" aria-hidden="true">
     <div class="case-design-stage">
       <div class="case-design-glow"></div>
       <div class="promo-case">
         <div class="promo-camera"></div>
         <div class="promo-photo"><span>YOUR</span><strong>PHOTO</strong></div>
         <div class="promo-signature">make it yours</div>
       </div>
       <div class="studio-tool tool-photo"><b>＋</b><span>Photo</span></div>
       <div class="studio-tool tool-text"><b>Aa</b><span>Text</span></div>
       <div class="studio-tool tool-color"><i></i><span>Color</span></div>
       <div class="studio-tool tool-preview"><b>↻</b><span>3D</span></div>
     </div>
   </div>
 </a>`;
...
function renderHomePromos(){
  ...
  const promoDots=[...section.querySelectorAll("[data-promo-dot]")];
  let promoIndex=0,promoTimer=0,resumeTimer=0,userTouching=false;
  const goPromo=(idx,smooth=true)=>{
    if(!track)return;
    promoIndex=(idx+promos.length)%promos.length;
    track.scrollTo({left:promoIndex*track.clientWidth,behavior:smooth?"smooth":"auto"});
    promoDots.forEach((d,n)=>d.classList.toggle("active",n===promoIndex));
  };
  const stopAuto=()=>{clearInterval(promoTimer);promoTimer=0};
  const startAuto=()=>{stopAuto();if(promos.length>1&&!document.hidden)promoTimer=setInterval(()=>goPromo(promoIndex+1,true),4200)};
  const resumeAuto=()=>{clearTimeout(resumeTimer);resumeTimer=setTimeout(startAuto,2200)};
  promoDots.forEach(b=>b.onclick=()=>{stopAuto();goPromo(Number(b.dataset.promoDot),true);resumeAuto()});
  if(track&&promos.length>1){
    let scrollRAF=0;
    track.addEventListener("scroll",()=>{if(scrollRAF)return;scrollRAF=requestAnimationFrame(()=>{scrollRAF=0;const i=Math.round(track.scrollLeft/Math.max(track.clientWidth,1));if(i!==promoIndex){promoIndex=Math.max(0,Math.min(promos.length-1,i));promoDots.forEach((d,n)=>d.classList.toggle("active",n===promoIndex))}})},{passive:true});
    track.addEventListener("touchstart",()=>{userTouching=true;stopAuto()},{passive:true});
    track.addEventListener("touchend",()=>{userTouching=false;resumeAuto()},{passive:true});
    track.addEventListener("pointerdown",()=>stopAuto(),{passive:true});
    track.addEventListener("pointerup",()=>resumeAuto(),{passive:true});
    document.addEventListener("visibilitychange",()=>document.hidden?stopAuto():startAuto());
    addEventListener("resize",()=>goPromo(promoIndex,false),{passive:true});
    startAuto();
  }
  observeReveals(section);
}
...
function injectStoreEnhancementStyles(){
  ...
  style.textContent=`
    ...
    .smart-phone-finder{padding-top:18px!important}.case-studio-showcase{position:relative;min-height:390px;display:grid;grid-template-columns:1fr 1fr;align-items:center;gap:20px;overflow:hidden;border-radius:30px;padding:clamp(25px,5vw,55px);text-decoration:none;background:radial-gradient(circle at 75% 35%,rgba(63,193,255,.22),transparent 32%),linear-gradient(135deg,#effbff 0%,#f3efff 56%,#fff 100%);border:1px solid rgba(46,127,255,.13);box-shadow:0 22px 65px rgba(36,79,143,.09);color:#0a1f44}.case-show-copy{position:relative;z-index:3}.case-show-copy h2{font-size:clamp(2rem,5vw,3.8rem);line-height:1.02;margin:8px 0 12px}.case-show-copy h2 span{background:linear-gradient(90deg,#1ba8ff,#7656ff);-webkit-background-clip:text;color:transparent}.case-show-copy p{max-width:520px;color:#718096;line-height:1.6}.case-show-cta{display:inline-flex;align-items:center;gap:10px;margin-top:18px;padding:13px 18px;border-radius:999px;background:linear-gradient(90deg,#1ea9ff,#6b4dff);color:#fff;font-weight:800;font-size:.75rem;box-shadow:0 12px 28px rgba(70,94,255,.22)}.case-show-visual{position:relative;height:300px}.promo-case{position:absolute;width:148px;height:286px;border-radius:30px;right:22%;top:5px;background:linear-gradient(145deg,#132a54,#294d8a);box-shadow:0 25px 45px rgba(20,48,91,.28);transform:rotate(8deg);animation:caseFloat 5s ease-in-out infinite alternate}.promo-camera{position:absolute;left:12px;right:12px;top:12px;height:82px;border-radius:22px;background:#eaf5ff;box-shadow:inset 0 0 0 7px #294d8a}.promo-art{position:absolute;left:18px;right:18px;top:125px;text-align:center;color:#fff;font-weight:900;font-size:1.45rem;letter-spacing:.08em}.promo-art small{font-size:.42rem;letter-spacing:.18em;color:#9be7ff}.float-tag{position:absolute;padding:8px 11px;border-radius:999px;background:rgba(255,255,255,.94);box-shadow:0 9px 25px rgba(24,62,110,.12);font-size:.58rem;font-weight:800;color:#17345e;animation:tagFloat 4s ease-in-out infinite alternate}.tag-photo{left:2%;top:24%}.tag-text{right:0;top:18%;animation-delay:.5s}.tag-color{left:4%;bottom:20%;animation-delay:1s}.tag-3d{right:1%;bottom:16%;animation-delay:1.5s}@keyframes caseFloat{to{transform:translateY(-12px) rotate(4deg)}}@keyframes tagFloat{to{transform:translateY(-7px)}}@media(max-width:700px){.case-studio-showcase{min-height:455px;grid-template-columns:1fr;padding:24px;border-radius:24px}.case-show-copy{text-align:left}.case-show-visual{height:220px}.promo-case{width:112px;height:214px;border-radius:24px;right:30%;top:0}.promo-camera{height:64px}.promo-art{top:100px;font-size:1.05rem}.float-tag{font-size:.52rem}.tag-photo{left:2%}.tag-text{right:3%}.tag-color{left:5%;bottom:12%}.tag-3d{right:1%;bottom:8%}}@media(max-width:700px){.phone-finder-card{border-radius:22px;padding:22px}.phone-finder-control{align-items:stretch;flex-direction:column}.phone-finder-control .text-btn{align-self:flex-start}.home-promo-card{aspect-ratio:4/5;border-radius:22px}.home-promo-shade{background:linear-gradient(0deg,rgba(3,18,43,.66),rgba(3,18,43,.02) 70%)}.home-promo-copy h2{font-size:1.8rem}.modal-product{grid-template-columns:1fr!important}}\n    /* V27: exact one-card promo viewport */\n    .home-promo-section{width:100%!important;overflow:hidden!important;padding:22px 0 10px!important}\n    .home-promo-track{display:flex!important;width:calc(100% - 40px)!important;max-width:1080px!important;margin:0 auto!important;padding:0!important;gap:0!important;overflow-x:auto!important;scroll-snap-type:x mandatory!important;scroll-padding:0!important;overscroll-behavior-x:contain!important;box-sizing:border-box!important}\n    .home-promo-card{flex:0 0 100%!important;width:100%!important;min-width:100%!important;max-width:100%!important;margin:0!important;scroll-snap-align:start!important;scroll-snap-stop:always!important;box-sizing:border-box!important}\n    .home-promo-card+ .home-promo-card{margin-left:0!important}
    /* V27.1: restore smooth homepage motion */
    .hero-device,.device-frame,.device-screen span,.device-screen strong,.device-screen small{backface-visibility:hidden;-webkit-backface-visibility:hidden;transform-style:preserve-3d}
    .reveal{will-change:transform,opacity;backface-visibility:hidden;-webkit-backface-visibility:hidden;transition:opacity .68s cubic-bezier(.22,.72,.2,1),transform .68s cubic-bezier(.22,.72,.2,1)!important}
    .product-grid .product-card.reveal:nth-child(2){transition-delay:.07s!important}.product-grid .product-card.reveal:nth-child(3){transition-delay:.14s!important}.product-grid .product-card.reveal:nth-child(4){transition-delay:.21s!important}
    .home-promo-track{scroll-behavior:smooth;-webkit-overflow-scrolling:touch}
    @media(prefers-reduced-motion:reduce){.reveal{transition:none!important}.product-grid .product-card.reveal{transition-delay:0s!important}}\n    /* V27: premium Case Studio artwork */\n    .case-show-cta{position:relative;overflow:hidden;padding:14px 20px!important;box-shadow:0 14px 32px rgba(65,82,255,.25)!important}.case-show-cta:after{content:"";position:absolute;inset:-60% -25%;background:linear-gradient(105deg,transparent 38%,rgba(255,255,255,.65) 50%,transparent 62%);animation:caseCtaShine 4s ease-in-out infinite}.case-show-cta span,.case-show-cta b{position:relative;z-index:1}@keyframes caseCtaShine{0%,62%{transform:translateX(-75%)}84%,100%{transform:translateX(75%)}}\n    .case-design-stage{position:relative;width:100%;height:100%;display:grid;place-items:center}.case-design-glow{position:absolute;width:210px;height:210px;border-radius:50%;background:radial-gradient(circle,rgba(64,181,255,.28),rgba(107,77,255,.10) 48%,transparent 72%);filter:blur(2px);animation:designGlow 4s ease-in-out infinite alternate}.promo-case{right:auto!important;top:auto!important;left:50%!important;transform:translateX(-50%) rotate(6deg)!important;background:linear-gradient(155deg,#102b59,#315b9c)!important;border:1px solid rgba(255,255,255,.35);overflow:hidden}.promo-camera{background:#eef8ff!important;box-shadow:inset 0 0 0 7px #315b9c!important}.promo-photo{position:absolute;left:14px;right:14px;top:108px;height:92px;border-radius:15px;background:linear-gradient(145deg,#68d8ff,#7866ff);display:flex;flex-direction:column;align-items:center;justify-content:center;color:white;box-shadow:inset 0 0 0 1px rgba(255,255,255,.28)}.promo-photo span{font-size:.45rem;letter-spacing:.22em;opacity:.82}.promo-photo strong{font-size:1rem;letter-spacing:.08em}.promo-signature{position:absolute;left:0;right:0;bottom:28px;text-align:center;color:#d8efff;font-size:.56rem;font-style:italic;letter-spacing:.04em}.studio-tool{position:absolute;display:flex;align-items:center;gap:6px;padding:8px 11px;border-radius:13px;background:rgba(255,255,255,.94);border:1px solid rgba(29,79,148,.08);box-shadow:0 10px 28px rgba(26,61,112,.14);color:#17345e;font-size:.55rem;font-weight:800;backdrop-filter:blur(10px);animation:toolFloat 4s ease-in-out infinite alternate}.studio-tool b{font-size:.7rem}.studio-tool i{width:12px;height:12px;border-radius:50%;background:linear-gradient(135deg,#1fb5ff,#7957ff);box-shadow:0 0 0 3px #eef5ff}.tool-photo{left:3%;top:25%}.tool-text{right:2%;top:18%;animation-delay:.4s}.tool-color{left:4%;bottom:17%;animation-delay:.8s}.tool-preview{right:1%;bottom:13%;animation-delay:1.2s}@keyframes toolFloat{to{transform:translateY(-6px)}}@keyframes designGlow{to{transform:scale(1.1);opacity:.75}}\n    @media(max-width:700px){.home-promo-track{width:calc(100% - 32px)!important}.home-promo-card{aspect-ratio:4/5!important;border-radius:22px!important}.case-studio-showcase{min-height:470px!important}.case-show-visual{height:230px!important}.case-design-glow{width:180px;height:180px}.promo-case{width:116px!important;height:220px!important}.promo-camera{height:65px!important}.promo-photo{top:88px;height:72px}.promo-signature{bottom:22px}.studio-tool{font-size:.5rem;padding:7px 9px}.tool-photo{left:1%;top:25%}.tool-text{right:1%;top:19%}.tool-color{left:3%;bottom:13%}.tool-preview{right:0;bottom:9%}}
  `;
