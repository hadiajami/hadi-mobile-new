...
let products=[],categories=[],phoneOptions=[],productImages=[],productColors=[],storageOptions=[],promos=[],brands=[],restockRequests=[],setupDevices=[],setupRenders=[],orders=[],orderItems=[];
...
async function reload(){
  const [c,p,o,i,col,st,pr,br,rr,sd,sr,ord,oi]=await Promise.all([
    ...
    sb.from("product_storage_options").select("*").order("sort_order"),
    ...
    sb.from("order_items").select("*").order("created_at",{ascending:true})
  ]);
  [c,p,o,i,col,st,pr,br,rr,sd,sr,ord,oi].forEach((r,idx)=>{if(r.error)console.error(["Categories","Products","Phone options","Images","Colors","Storage","Promos","Brands","Requests","Setup devices","Setup renders","Orders","Order items"][idx],r.error);});
  categories=c.data||[];products=p.data||[];phoneOptions=o.data||[];productImages=i.data||[];productColors=col.data||[];storageOptions=st.data||[];promos=pr.data||[];brands=br.data||[];restockRequests=rr.data||[];setupDevices=sd.data||[];setupRenders=sr.data||[];orders=ord.data||[];orderItems=oi.data||[];
  render();
}
...
const storageFor=id=>storageOptions.filter(o=>String(o.product_id)===String(id));
...
function render(){
  ...
  if(orderList){
    orderList.innerHTML=orders.length?orders.map(o=>{
      ...
      return `<div class="order-card">
        <div class="order-top"><div><span class="order-code">${esc(o.order_code)}</span><strong>${esc(o.full_name)}</strong><small>${new Date(o.created_at).toLocaleString()}</small></div><select class="order-status" data-order-status="${o.id}"><option value="new" ${o.status==="new"?"selected":""}>New</option><option value="confirmed" ${o.status==="confirmed"?"selected":""}>Confirmed</option><option value="preparing" ${o.status==="preparing"?"selected":""}>Preparing</option><option value="out_for_delivery" ${o.status==="out_for_delivery"?"selected":""}>Out for delivery</option><option value="delivered" ${o.status==="delivered"?"selected":""}>Delivered</option><option value="cancelled" ${o.status==="cancelled"?"selected":""}>Cancelled</option></select></div>
        <div class="order-items">${items.map(i=>`<div class="order-item"><img src="${i.image_url||"../assets/img/hadi-mobile-icon.jpg"}"><div><strong>${esc(i.product_name)}</strong><small>${i.phone_model?esc(i.phone_model)+" · ":""}${i.color_name?esc(i.color_name)+" · ":""}${i.storage_name?esc(i.storage_name)+" · ":""}Qty ${i.quantity}</small></div><b>$${(Number(i.unit_price)*Number(i.quantity)).toFixed(2)}</b></div>`).join("")}</div>
  ...
  $("#productList").innerHTML=products.map(p=>{
    const opts=optionsFor(p.id), imgs=imagesFor(p.id), cols=colorsFor(p.id), stores=storageFor(p.id);
...
$("#newProduct").onclick=()=>{
  ...
  $("#productHasPhoneOptions").checked=false;$("#productHasColors").checked=false;$("#productHasStorage").checked=false;$("#productEditorTitle").textContent="Add product";
  $("#phoneOptionsRows").innerHTML="";$("#colorOptionsRows").innerHTML="";$("#storageOptionsRows").innerHTML="";editingGallery=[];renderImageManager();toggleDrawers();$("#productEditor").classList.add("open");
};
$("#newPromo").onclick=()=>{
...
function editProduct(id){
  ...
  $("#productStock").checked=!!p.in_stock;$("#productFeatured").checked=!!p.featured;$("#productNewArrival").checked=!!p.is_new_arrival;$("#productTrending").checked=!!p.is_trending;$("#productActive").checked=p.is_active!==false;$("#productHasPhoneOptions").checked=!!p.has_phone_options;$("#productHasColors").checked=!!p.has_color_options;$("#productHasStorage").checked=!!p.has_storage_options;$("#productEditorTitle").textContent="Edit product";
  ...
  $("#storageOptionsRows").innerHTML="";storageFor(p.id).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).forEach(st=>addStorageRow(st.name,st.price_override,st.in_stock));
...
$("#productHasStorage").addEventListener("change",()=>{if($("#productHasStorage").checked&&!$("#storageOptionsRows").children.length)addStorageRow("256GB",null,true);toggleDrawers();});
$("#addPhoneOption").onclick=()=>addPhoneRow("",true);
$("#addColorOption").onclick=()=>{addColorRow("","#111111");renderImageManager();};
$("#addStorageOption").onclick=()=>addStorageRow("",null,true);
...
function addStorageRow(name="",priceOverride=null,inStock=true){
  const row=document.createElement("div");row.className="storage-option-row";row.innerHTML=`<input class="storage-name-input" type="text" placeholder="e.g. 256GB" value="${esc(name)}"><input class="storage-price-input" type="number" min="0" step="0.01" placeholder="Price override" value="${priceOverride??""}"><label class="phone-stock-switch"><input class="storage-stock-input" type="checkbox" ${inStock?"checked":""}><span>In stock</span></label><button class="phone-remove-btn" type="button">×</button>`;row.querySelector(".phone-remove-btn").onclick=()=>row.remove();$("#storageOptionsRows").appendChild(row);
}
...
function toggleDrawers(){
  const phones=$("#productHasPhoneOptions").checked, colors=$("#productHasColors").checked, storage=$("#productHasStorage").checked;$("#phoneOptionsDrawer").classList.toggle("hidden",!phones);$("#colorOptionsDrawer").classList.toggle("hidden",!colors);$("#storageOptionsDrawer").classList.toggle("hidden",!storage);$("#productStock").disabled=phones||storage;const l=$("#productStock").closest("label");if(l)l.style.opacity=(phones||storage)?".45":"1";
}
...
const getStorageRows=()=>[...document.querySelectorAll(".storage-option-row")].map((row,index)=>({name:row.querySelector(".storage-name-input").value.trim(),price_override:row.querySelector(".storage-price-input").value.trim()?Number(row.querySelector(".storage-price-input").value):null,in_stock:row.querySelector(".storage-stock-input").checked,sort_order:index})).filter(r=>r.name);
...
$("#productForm").addEventListener("submit",async e=>{
  ...
  try{
    const hasPhones=$("#productHasPhoneOptions").checked, hasColors=$("#productHasColors").checked, hasStorage=$("#productHasStorage").checked;const phones=hasPhones?getPhoneRows():[];const colors=hasColors?getColorRows():[];const storages=hasStorage?getStorageRows():[];
    ...
    if(hasStorage&&!storages.length)throw new Error("Add at least one storage option.");
    ...
    if(new Set(storages.map(r=>r.name.toLowerCase())).size!==storages.length)throw new Error("The same storage option was added more than once.");
    ...
    r=await sb.from("product_storage_options").delete().eq("product_id",productId);if(r.error)throw r.error;
    ...
    if(hasStorage&&storages.length){r=await sb.from("product_storage_options").insert(storages.map(x=>({...x,product_id:productId})));if(r.error)throw r.error;}
