  ...
  if(orderList){
    orderList.innerHTML=orders.length?orders.map(o=>{
      ...
      return `<div class="order-card">
        ...
        <div class="order-items">${items.map(i=>`<div class="order-item"><img src="${i.image_url||"../assets/img/hadi-mobile-icon.jpg"}"><div><strong>${esc(i.product_name)}</strong><small>${i.phone_model?esc(i.phone_model)+" · ":""}${i.color_name?esc(i.color_name)+" · ":""}Qty ${i.quantity}${i.custom_design_id?` · <a href="#case-${i.custom_design_id}" data-open-case="${i.custom_design_id}">Open custom design</a>`:""}</small></div><b>$${(Number(i.unit_price)*Number(i.quantity)).toFixed(2)}</b></div>`).join("")}</div>
