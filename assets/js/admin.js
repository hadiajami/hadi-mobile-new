...
let products=[],categories=[],phoneOptions=[],productImages=[],productColors=[],promos=[],brands=[],restockRequests=[],setupDevices=[],setupRenders=[],orders=[],orderItems=[];
...
async function reload(){
  const [c,p,o,i,col,pr,br,rr,sd,sr,ord,oi]=await Promise.all([
  ...
  [c,p,o,i,col,pr,br,rr,sd,sr,ord,oi].forEach((r,idx)=>{if(r.error)console.error(["Categories","Products","Phone options","Images","Colors","Promos","Brands","Requests","Setup devices","Setup renders","Orders","Order items"][idx],r.error);});
  categories=c.data||[];products=p.data||[];phoneOptions=o.data||[];productImages=i.data||[];productColors=col.data||[];promos=pr.data||[];brands=br.data||[];restockRequests=rr.data||[];setupDevices=sd.data||[];setupRenders=sr.data||[];orders=ord.data||[];orderItems=oi.data||[];
  render();
}
