...
function render(){
  ...
  const ps=customCaseSettings.find(x=>Number(x.id)===1);if($("#customCasePrice"))$("#customCasePrice").value=ps?.price??"";if($("#customCaseDesignList"))$("#customCaseDesignList").innerHTML=customCaseDesigns.length?customCaseDesigns.map(d=>{const originals=Array.isArray(d.original_files)?d.original_files:[];return `<div class="case-order-card" id="case-${d.id}"><img src="${d.preview_url||"../assets/img/hadi-mobile-icon.jpg"}"><div class="case-order-main"><strong>${esc(d.phone_model||"Custom case")}</strong><small>${new Date(d.created_at).toLocaleString()} · ${esc(d.case_color||"")}</small><div class="case-files"><a class="mini-btn" href="${d.preview_url}" target="_blank">Final preview</a>${originals.map((f,n)=>`<a class="mini-btn" href="${f.url}" target="_blank">Original photo ${n+1}</a>`).join("")}</div></div><select class="case-design-status status-${esc(d.status||"pending")}" data-case-status="${d.id}"><option value="pending" ${d.status==="pending"?"selected":""}>Pending</option><option value="in_progress" ${d.status==="in_progress"?"selected":""}>In progress</option><option value="done" ${d.status==="done"?"selected":""}>Done</option></select></div>`}).join(""):`<div class="empty-admin">No custom case designs yet.</div>`;
...
$("#saveCustomCasePrice")?.addEventListener("click",async()=>{const price=Number($("#customCasePrice").value||0);if(price<=0){alert("Enter the custom case selling price.");return}const r=await sb.from("custom_case_settings").upsert({id:1,price,updated_at:new Date().toISOString()});if(r.error)alert(r.error.message);else alert("Custom case price saved.")});
