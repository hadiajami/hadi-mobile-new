...
let products=[],categories=[],phoneOptions=[],productImages=[],productColors=[],promos=[],brands=[],restockRequests=[],setupDevices=[],setupRenders=[],orders=[],orderItems=[],customCaseDesigns=[],customCaseSettings=[];
...
async function reload(){
  const [c,p,o,i,col,pr,br,rr,sd,sr,ord,oi,ccd,ccs]=await Promise.all([
    ...
    sb.from("order_items").select("*").order("created_at",{ascending:true}),sb.from("custom_case_designs").select("*").order("created_at",{ascending:false}),sb.from("custom_case_settings").select("*")
  ]);
  [c,p,o,i,col,pr,br,rr,sd,sr,ord,oi,ccd,ccs].forEach((r,idx)=>{if(r.error)console.error(["Categories","Products","Phone options","Images","Colors","Promos","Brands","Requests","Setup devices","Setup renders","Orders","Order items","Custom cases","Case settings"][idx],r.error);});
  categories=c.data||[];products=p.data||[];phoneOptions=o.data||[];productImages=i.data||[];productColors=col.data||[];promos=pr.data||[];brands=br.data||[];restockRequests=rr.data||[];setupDevices=sd.data||[];setupRenders=sr.data||[];orders=ord.data||[];orderItems=oi.data||[];customCaseDesigns=ccd.data||[];customCaseSettings=ccs.data||[];
  render();
}
...
function customDesignFor(id){return customCaseDesigns.find(d=>String(d.id)===String(id))||null}
function renderCustomOrderDetails(item){
  if(!item.custom_design_id)return "";
  const d=customDesignFor(item.custom_design_id);if(!d)return `<div class="order-custom-missing">Custom design record not found.</div>`;
  const originals=Array.isArray(d.original_files)?d.original_files:[];
  const texts=Array.isArray(d.text_layers)?d.text_layers:[];
  return `<div class="order-custom-box">
    <div class="order-custom-head"><strong>Customer customization</strong><a href="#case-${esc(d.id)}" data-open-case="${esc(d.id)}">Open in Case Studio</a></div>
    <div class="order-custom-media">
      ${d.preview_url?`<a class="order-custom-preview" href="${esc(d.preview_url)}" target="_blank" rel="noopener"><img src="${esc(d.preview_url)}" alt="Saved 2D rear preview"><span>Saved 2D rear preview</span></a>`:""}
      ${originals.map((f,n)=>`<a class="order-custom-original" href="${esc(f.url||"")}" target="_blank" rel="noopener"><img src="${esc(f.url||"")}" alt="Original photo ${n+1}"><span>Original photo ${n+1}<small>Open full quality</small></span></a>`).join("")}
    </div>
    ${texts.length?`<div class="order-custom-texts">${texts.map((t,n)=>`<div><b>Text ${n+1}:</b> <span>${esc(t.text||"")}</span><small>${esc(t.font||"Manrope")} · ${esc(t.color||"")}</small></div>`).join("")}</div>`:""}
  </div>`;
}
...
function render(){
  $("#stats").innerHTML=`
    ...
    <div class="stat"><strong>${orders.filter(o=>o.status==="new").length}</strong><span>NEW ORDERS</span></div>
  ...
  if(orderList){
    orderList.innerHTML=orders.length?orders.map(o=>{
      ...
      return `<div class="order-card">
        <div class="order-top"><div><span class="order-code">${esc(o.order_code)}</span><strong>${esc(o.full_name)}</strong><small>${new Date(o.created_at).toLocaleString()}</small></div><select class="order-status" data-order-status="${o.id}"><option value="new" ${o.status==="new"?"selected":""}>Pending</option><option value="confirmed" ${o.status==="confirmed"?"selected":""}>Confirmed</option><option value="preparing" ${o.status==="preparing"?"selected":""}>In progress</option><option value="out_for_delivery" ${o.status==="out_for_delivery"?"selected":""}>Ready / delivery</option><option value="delivered" ${o.status==="delivered"?"selected":""}>Done</option><option value="cancelled" ${o.status==="cancelled"?"selected":""}>Cancelled</option></select></div>
        <div class="order-items">${items.map(i=>`<div class="order-item-wrap"><div class="order-item"><img src="${i.image_url||"../assets/img/hadi-mobile-icon.jpg"}"><div><strong>${esc(i.product_name)}</strong><small>${i.phone_model?esc(i.phone_model)+" · ":""}${i.color_name?esc(i.color_name)+" · ":""}Qty ${i.quantity}</small></div><b>$${(Number(i.unit_price)*Number(i.quantity)).toFixed(2)}</b></div>${renderCustomOrderDetails(i)}</div>`).join("")}</div>
        <div class="order-info"><p><b>Phone:</b> ${esc(o.phone)}</p><p><b>Payment:</b> ${payment}</p><p><b>Address:</b> ${esc(address)}</p>${extras?`<p>${extras}</p>`:""}<p><b>Subtotal:</b> $${Number(o.subtotal||0).toFixed(2)} · <b>Delivery:</b> ${o.delivery_fee==null?"Not set":`$${Number(o.delivery_fee).toFixed(2)}`}</p></div>
        <div class="order-actions"><a class="mini-btn" href="tel:${esc(o.phone)}">Call</a><a class="mini-btn" target="_blank" rel="noopener" href="https://wa.me/${phone}?text=${waMsg}">WhatsApp customer</a></div>
      </div>`;
    }).join(""):`<div class="empty-admin">No website orders yet.</div>`;
  }
  ...
  const ps=customCaseSettings.find(x=>Number(x.id)===1);if($("#customCaseTextPrice"))$("#customCaseTextPrice").value=ps?.text_price??5;if($("#customCaseImagePrice"))$("#customCaseImagePrice").value=ps?.image_price??10;if($("#customCaseDesignList"))$("#customCaseDesignList").innerHTML=customCaseDesigns.length?customCaseDesigns.map(d=>{const originals=Array.isArray(d.original_files)?d.original_files:[];return `<div class="case-order-card" id="case-${d.id}"><img src="${d.preview_url||"../assets/img/hadi-mobile-icon.jpg"}"><div class="case-order-main"><strong>${esc(d.phone_model||"Custom case")}</strong><small>${new Date(d.created_at).toLocaleString()} · ${esc(d.case_color||"")}</small><div class="case-files">${d.preview_url?`<a class="mini-btn" href="${d.preview_url}" target="_blank">Final preview</a>`:""}${originals.map((f,n)=>`<a class="mini-btn" href="${f.url}" target="_blank" download>Original photo ${n+1}</a>`).join("")}</div>${Array.isArray(d.text_layers)&&d.text_layers.length?`<div style="margin-top:8px">${d.text_layers.map((t,n)=>`<div><b>Text ${n+1}:</b> ${esc(t.text||"")} <small>· ${esc(t.font||"")} · ${esc(t.color||"")}</small></div>`).join("")}</div>`:""}</div><select class="case-design-status status-${esc(d.status||"pending")}" data-case-status="${d.id}"><option value="pending" ${d.status==="pending"?"selected":""}>Pending</option><option value="in_progress" ${d.status==="in_progress"?"selected":""}>In progress</option><option value="done" ${d.status==="done"?"selected":""}>Done</option></select></div>`}).join(""):`<div class="empty-admin">No custom case designs yet.</div>`;
...
function bind(){document.querySelectorAll("[data-case-status]").forEach(s=>s.onchange=()=>updateCaseDesignStatus(s.dataset.caseStatus,s.value,s));document.querySelectorAll("[data-open-case]").forEach(a=>a.onclick=e=>{e.preventDefault();document.querySelector('[data-tab="setup"]')?.click();setTimeout(()=>document.getElementById("case-"+a.dataset.openCase)?.scrollIntoView({behavior:"smooth",block:"center"}),100)});
...
async function updateOrderStatus(id,status,el){
  el.disabled=true;
  const r=await sb.from("orders").update({status,updated_at:new Date().toISOString()}).eq("id",id);
  if(r.error)alert(r.error.message);
  else{const o=orders.find(x=>String(x.id)===String(id));if(o)o.status=status;}
  el.disabled=false;
}
